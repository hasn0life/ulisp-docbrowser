...
//use this version of superprint to avoid escape characters in the string when pretty printing
void superprint (object *form, int lm, bool match,  pfun_t pfun) {
  if (atom(form)) {
...
void decodeKeyStatus () {
  ...
  for (auto i : status.word) {
    // if ((status.fn) && (i == ' ')) autoComplete();
    //else { 
      ProcessKey(i); LastKeyword = 0; 
    //}
  }
  //if (status.enter) { ProcessKey('\n'); LastKeyword = 0; }
  if (status.enter && status.shift) { ProcessKey('\t'); LastKeyword = 0; }
  else if (status.enter) { ProcessKey('\n'); LastKeyword = 0; }
  if (status.del) { ProcessKey(8); LastKeyword = 0; }
  if (status.tab) autoComplete();
  //if (status.tab) { ProcessKey('\t'); LastKeyword = 0; }
}
...
void autoComplete(){
  ...
  if(LastKeyword == 0) { 
    ...
    for(matchLen = 0; matchLen < 32; matchLen++){
      ...
      if((KybdBuf[bufLoc] == ' ') || 
        (KybdBuf[bufLoc] == '(') || 
        (KybdBuf[bufLoc] == '\n')){
          //move past those characters cause we're not matching on them
        bufIndex = bufLoc + 1;
        matchLen--;
        break;
      }
      //this test has to come after in case the first character in the buffer is one of the charcaters we test for
      else if(bufLoc == 0){ 
        bufIndex = bufLoc; 
        break; 
      } 
  ...
  int entries = tablesize(0);
  while (true) {
    char *k = (char*)(table(0)[i].string);
    i = (i + 1) % entries; // Wrap
    if (*k == KybdBuf[bufIndex]) {
      if(strncmp(k, &KybdBuf[bufIndex], matchLen) == 0){
        //skip the letters we're matching cause they're already there
    ...
    gap++; 
    if (gap == entries) {  // No keywords with this letter
      return;
    } 
  }
}

// void InsertKeyword (char c) {
//   static int i = 0;
//   int gap = 0;
//   for (int n=0; n<LastKeyword; n++) ProcessKey(8);
//   int entries = tablesize(0);
//   while (true) {
//     char *k = (char*)(table(0)[i].string);
//     gap++; if (gap == entries) return; // No keywords with this letter
//     i = (i+1) % entries; // Wrap
//     if (*k == c) {
//       LastKeyword = strlen(k);
//       while (*k) ProcessKey(*k++);
//       return;
//     }
//   }
// }
